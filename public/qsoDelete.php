...
// -------
//
function qruqsp_winterfielddaylog_qsoDelete(&$ciniki) {
    //
    ...
    $rc = qruqsp_winterfielddaylog_checkAccess($ciniki, $args['tnid'], 'ciniki.winterfielddaylog.qsoDelete');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( !isset($rc['qso']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.winterfielddaylog.4', 'msg'=>'QSO does not exist.'));
    }
    $qso = $rc['qso'];
    ...
    $rc = ciniki_core_objectCheckUsed($ciniki, $args['tnid'], 'qruqsp.winterfielddaylog.qso', $args['qso_id']);
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.winterfielddaylog.14', 'msg'=>'Unable to check if the qso is still being used.', 'err'=>$rc['err']));
    }
    if( $rc['used'] != 'no' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.winterfielddaylog.6', 'msg'=>'The qso is still in use. ' . $rc['msg']));
    }
    ...
    // Remove the qso
    //
    $rc = ciniki_core_objectDelete($ciniki, $args['tnid'], 'qruqsp.winterfielddaylog.qso',
