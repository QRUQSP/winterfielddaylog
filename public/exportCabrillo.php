...
function qruqsp_winterfielddaylog_exportCabrillo($ciniki) {
    ...
    foreach($qsos as $qso) {
        ...
        if( !in_array($qso['operator'], $operators) ) {
            $operators[] = $qso['operator'];
        }
    }
    ...
    $cabrillo .= "Created-By: QRUQSP.org WinterFieldDayLogger2021\r\n";
...
//    $cabrillo .= "CATEGORY-TRANSMITTER: " . (isset($settings['category-transmitter']) ? $settings['category-transmitter'] : '') . "\r\n";
    ...
    header('Content-Disposition: attachment; filename="winterfieldday2021.log"');
