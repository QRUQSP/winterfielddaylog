...
// Binary image data
//
function qruqsp_winterfielddaylog_mapGet(&$ciniki) {
    //
    // Check args
    //
...
//        'sections'=>array('required'=>'no', 'blank'=>'yes', 'type'=>'list', 'name'=>'Sections'), 
    ...
    $rc = qruqsp_winterfielddaylog_checkAccess($ciniki, $args['tnid'], 'qruqsp.winterfielddaylog.mapGet');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.winterfielddaylog.18', 'msg'=>'', 'err'=>$rc['err']));
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.winterfielddaylog.26', 'msg'=>'', 'err'=>$rc['err']));
    }
...
/*        $map = imagecreatefrompng($ciniki['config']['qruqsp.core']['modules_dir'] . '/winterfielddaylog/maps/back_with_lines.png');
        if( isset($args['sections'][0]) && $args['sections'][0] != '' ) {
            foreach($args['sections'] as $s) {
                if( file_exists($ciniki['config']['qruqsp.core']['modules_dir'] . '/winterfielddaylog/maps/' . $s . '.png') ) {
                    $overlay = imagecreatefrompng($ciniki['config']['qruqsp.core']['modules_dir'] . '/winterfielddaylog/maps/' . $s . '.png');
...
//        $ciniki['session']['qruqsp.winterfielddaylog']['map_sections'] = $sections;
